...
import { BufferGeometryUtils } from 'https://unpkg.com/three@0.118.3/examples/jsm/utils/BufferGeometryUtils.js';//
...
function recrearArbol(){
    ...
    /*  let cadena = crearArbol( 1 , 3 , angulos , THREE.MathUtils.randInt(2,5) , largos , puntas , segmentos , hijos , 1.0 , 0.8 , 1.0 );
      ...
      for(let i=0; i<100;i++){
          ...
          var arbolGeo = new GeometriaArbol(cadena,scene);
          var geometria = arbolGeo.crear();
          geometria.translate ( THREE.MathUtils.randFloat(-300,300), -50, THREE.MathUtils.randFloat(-300,300) );
          geos.push(geometria);
      ...
      if(arbol==undefined){
          arbol = new THREE.Mesh( geo, material );
      }else{
          arbol.geometry.dispose();
          arbol.geometry.copy(geo);
      }
      arbol.scale.x = 4;
      arbol.scale.y = 4;
      arbol.scale.z = 4;
